...
(function(factory) {
	if (typeof define === "function" && define.amd) {
		...
		(function($, window, document) {
			...
			Checkboxes.prototype = {
				_constructor : function() {
					...
					for (var i = 0; i < ctx.aoColumns.length; i++) {
						if (ctx.aoColumns[i].checkboxes) {
							...
							if (state && state.checkboxes
									&& state.checkboxes.hasOwnProperty(i)) {
				...
				showInfoSelected : function() {
					...
					var add = function(name, num) {
						$output.append($('<span class="select-item"/>').append(
			...
			Api.registerPlural("columns().checkboxes.selected()",
					"column().checkboxes.selected()", function() {
						return this.iterator("column-rows", function(ctx,
								colIdx, i, j, rowsIdx) {
							if (ctx.aoColumns[colIdx].checkboxes) {
								var selector = [];
								...
								$.each(cellsData, function(index, cellData) {
									if (ctx.checkboxes.s.data[colIdx]
											.hasOwnProperty(cellData)) {
										if (ctx.checkboxes.isCellSelectable(
												colIdx, cellData)) {
											data.push(cellData);
										}
									}
